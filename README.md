...
| **Dashboard — Error 500 al cargar** | El endpoint `/api/data/dashboard` podía devolver 500 si las columnas `fecha` / `fecha_reunion` de agendas llegaban como `Date` en lugar de string. En `volumeByDay.sort()` se usa `localeCompare`, que es de string; con `Date` se lanzaba `TypeError`. Se añadió `toDateString()` en `dashboard.ts` para normalizar todas las fechas a `"YYYY-MM-DD"` antes de usarlas como clave y en el sort. |
| **Dashboard — Error 500 por toLowerCase en undefined** | Si `embudo_personalizado` o datos de BD tenían `nombre`/`tipo_evento`/objeciones undefined, el dashboard lanzaba `TypeError: Cannot read properties of undefined (reading 'toLowerCase')` en `buildFunnelSets` o en filtros. Se endureció `buildFunnelSets` (nombres seguros y `(n ?? "").toLowerCase()`), objeciones (`obj?.categoria ?? obj?.objecion`) y llamadas (`(c.tipo_evento ?? "").startsWith(...)`). Las respuestas 500 del dashboard incluyen cuerpo JSON con `error` y `debug` para depuración; el frontend muestra el mensaje y botón "Reintentar". |
