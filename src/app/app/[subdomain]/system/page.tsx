...
import { ChevronRight, ChevronLeft, Phone, Video, Tag, BarChart3, Building2, Save, Target, Loader2, Key, GitBranch, MessageSquare, Database, Plus, Trash2, GripVertical, ArrowRight, Pencil, HelpCircle, AlertTriangle, BookOpen } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
      ...
      <div className="p-3 md:p-4 max-w-3xl mx-auto space-y-4 text-sm min-w-0 max-w-full overflow-x-hidden">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-2">
          {[
            ...
            const isBetaStep: boolean = false;
            return (
              ...
              <div className="mt-6 pt-5 border-t border-surface-500">
                ...
                {categoriasLlamadas.length > 0 && (
                  <ul className="space-y-2 mb-3">
                    {categoriasLlamadas.map((cat) => {
                      const isRouted = tagRules.some((r) => r.acciones.some((a) => a.tipo === 'asignar_categoria' && a.categoria_id === cat.id));
                      return (
                        <li key={cat.id} className="rounded-xl p-3 border-l-4 border-accent-purple/60 bg-gradient-to-b from-surface-700/90 to-surface-800/90 border border-surface-500 flex items-start gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            {!isRouted && (
                              <div className="flex items-center gap-1 mt-1.5 px-2 py-1 rounded-md bg-amber-500/10 border border-amber-500/30 w-fit">
                                <AlertTriangle className="w-3 h-3 text-amber-400 shrink-0" />
                                <span className="text-[10px] text-amber-400">Ninguna regla enruta esta categoría — no se usará</span>
                              </div>
                            )}
                          </div>
                ...
                {catEditId !== null ? (
                  <div className="rounded-xl p-4 border border-accent-purple/30 bg-surface-700/50 space-y-3">
                    <div>
                      <label className="flex items-center gap-1 text-[11px] font-medium text-accent-purple mb-1">
                        Nombre de la categoría
                        <HelpTooltip titulo="Nombre de la categoría" contenido="Identificador corto que verás en listas y reportes. No afecta la evaluación de la IA, solo sirve para que tú identifiques cada tipo de llamada." />
                      </label>
                    ...
                    <div>
                      <label className="flex items-center gap-1 text-[11px] font-medium text-accent-purple mb-1">
                        Definición
                        <HelpTooltip titulo="Definición de la categoría" contenido="Describe en 1-2 oraciones qué tipo de llamada representa esta categoría. La IA usa esta definición como contexto adicional al evaluar la llamada con el prompt." />
                      </label>
                      <textarea value={catDefinicion} onChange={(e) => setCatDefinicion(e.target.value)}
                        placeholder="Ej: Llamada donde se busca entender las necesidades del lead por primera vez."
                    ...
                    <div>
                      <label className="flex items-center gap-1 text-[11px] font-medium text-accent-purple mb-1">
                        Temas
                        <HelpTooltip titulo="Temas de la categoría" contenido="Lista de temas que la IA evaluará en esta categoría. Cada tema se convierte en una puntuación individual en el reporte de la llamada. Ej: 'Identificación de necesidades', 'Manejo de objeciones'." />
                      </label>
                    ...
                    <div>
                      <label className="flex items-center gap-1 text-[11px] font-medium text-accent-purple mb-1">
                        Prompt de evaluación
                        <HelpTooltip titulo="Prompt de evaluación" contenido="Instrucciones que la IA sigue al analizar cada llamada de esta categoría. Escribe exactamente cómo quieres que se evalúe: criterios, escala, qué buscar. Este prompt reemplaza al prompt general solo para las llamadas asignadas a esta categoría." />
                      </label>
                  ...
                  <div className="space-y-3">
                    ...
                    {categoriasLlamadas.length === 0 && (
                      <div className="rounded-xl border border-dashed border-surface-500 bg-surface-800/50 p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <BookOpen className="w-4 h-4 text-accent-purple" />
                          <p className="text-xs font-medium text-gray-300">Plantillas de ejemplo</p>
                        </div>
                        <p className="text-[11px] text-gray-500 mb-3">Haz click en una plantilla para precargarla. Puedes editar todo antes de guardar.</p>
                        <div className="grid gap-2 sm:grid-cols-3">
                          {([
                            { nombre: 'Descubrimiento', definicion: 'Primera llamada con el lead para entender sus necesidades y contexto.', temas: ['Identificación de necesidades', 'Contexto del lead', 'Siguiente paso acordado'], prompt: 'Evalúa esta llamada de descubrimiento. ¿El vendedor identificó las necesidades principales del lead? ¿Estableció un siguiente paso claro?' },
                            { nombre: 'Perfilamiento', definicion: 'Llamada donde se profundiza en el perfil del lead: presupuesto, timeline y tomadores de decisión.', temas: ['Presupuesto', 'Timeline', 'Tomador de decisión', 'Calificación'], prompt: 'Evalúa esta llamada de perfilamiento. ¿El vendedor obtuvo información sobre presupuesto, timeline y quién toma la decisión? ¿El lead quedó calificado?' },
                            { nombre: 'Cierre', definicion: 'Llamada enfocada en cerrar la venta: presentar propuesta, manejar objeciones y obtener compromiso.', temas: ['Presentación de propuesta', 'Manejo de objeciones', 'Compromiso obtenido'], prompt: 'Evalúa esta llamada de cierre. ¿El vendedor presentó la propuesta con claridad? ¿Manejó las objeciones? ¿Obtuvo un compromiso del lead?' },
                          ] as const).map((tpl) => (
                            <button key={tpl.nombre} type="button" onClick={() => {
                              setCatEditId(crypto.randomUUID());
                              setCatNombre(tpl.nombre);
                              setCatDefinicion(tpl.definicion);
                              setCatTemas([...tpl.temas]);
                              setCatPrompt(tpl.prompt);
                              setCatTemaInput('');
                            }} className="text-left rounded-lg p-2.5 border border-surface-500 hover:border-accent-purple/40 bg-surface-700/50 hover:bg-surface-700 transition-all group">
                              <p className="text-xs font-medium text-white group-hover:text-accent-purple">{tpl.nombre}</p>
                              <p className="text-[10px] text-gray-500 mt-0.5 line-clamp-2">{tpl.definicion}</p>
                            </button>
                          ))}
                        </div>
                      </div>
